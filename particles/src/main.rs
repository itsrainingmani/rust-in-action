...
unsafe impl GlobalAlloc for ReportingAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        // defers actual mem alloc to the system's default mem allocator
...
impl Particle {
    ...
    fn update(&mut self) {
        ...
        // makes particle more transparent over time
        self.color[3] *= 0.995;
    }
}
...
impl World {
    fn new(width: f64, height: f64) -> World {
        World {
            current_turn: 0,
            particles: Vec::<Box<Particle>>::new(), // use Box<Particle> to incur an extra mem alloc
            height,
            width,
            rng: thread_rng(),
        }
